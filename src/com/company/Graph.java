...
public class Graph {
    ...
    public int path() { // выбор кратчайшего пути
    ...
    private int displayPaths() { // метод для вывода кратчайшего пути на экран
        if (shortestPaths.get(countOfVertices - 1).getDistance() == INFINITY) {
            return 0;
        } else {
            return shortestPaths.get(countOfVertices - 1).getDistance();
        }
    }
}
